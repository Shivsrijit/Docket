...
import { X, Save, Trash2, FolderKanban, Loader2, Star, Bold, Italic, Highlighter, List, ListTodo, Link2, Heading1, Heading2, Quote, Code, Eraser } from "lucide-react";
import ColorPaletteSelector from "../components/ColorPaletteSelector";

const NoteDetailPage = ({ id, onClose, onUpdated }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        // Fetch note and folders in parallel
  ...
  // Set innerHTML once loaded
  useEffect(() => {
    if (!loading && note && editorRef.current && !initialSet.current) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error("Error in delete note flow", error);
      ...
      <div className="relative w-full max-w-4xl h-[90vh] md:h-[82vh] bg-white dark:bg-zinc-950 border border-neutral-100 dark:border-zinc-800 shadow-2xl rounded-[2.5rem] flex flex-col z-10 overflow-hidden transition-all duration-300">
        ...
        {loading || !note ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 pt-10">
            <Loader2 className="animate-spin size-8 text-neutral-800 dark:text-zinc-200" />
          ...
          <div className="flex-1 overflow-y-auto px-6 sm:px-8 md:px-12 pt-10 pb-8 space-y-6">
            ...
            {/* Borderless Large Title Input with Star Pin, Delete, and Close Toggles */}
              ...
              <div className="flex items-center gap-2">
                {note && (
                  <button 
                    ...
                    disabled={deleting}
                    className="size-11 sm:size-12 rounded-2xl bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-500 flex items-center justify-center hover:scale-105 active:scale-95 transition-all cursor-pointer border border-red-100 dark:border-red-500/20"
                    title="Delete note"
                  >
                ...
                <button
                  ...
                  title={note.isPinned ? "Unpin Docket" : "Pin Docket"}
                >
                  <Star className="size-5" fill={note.isPinned ? "currentColor" : "none"} />
                </button>
                <button
                  type="button"
                  onClick={handleClose}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-neutral-100 dark:border-zinc-900 pt-6">
              ...
              <ColorPaletteSelector 
                ...
                onChange={(colorValue) => setNote({ ...note, color: colorValue })} 
              />

              {/* Folder & Pin Config Panel */}
